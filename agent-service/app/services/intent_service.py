...
KNOWLEDGE_KEYWORDS = (
    "工作时间", "几点上班", "几点下班", "客服时间", "上班时间", "下班时间",
    "退货规则", "退货原则", "七天无理由", "退货条件", "退货流程",
    "换货规则", "换货条件", "换货流程",
    "退款规则", "退款原则",
    "商品材质", "商品规格", "怎么使用", "使用方法", "保养", "洗涤",
    "企业文化", "品牌介绍", "公司介绍", "品牌故事", "什么文化",
    "会员", "优惠券", "积分规则", "积分", "会员等级",
    "保修", "保修期", "保修政策", "售后政策", "售后服务",
    "发货规则", "配送范围", "物流时效", "几天到货",
    "发票规则", "开发票", "发票抬头",
    "客服在线", "人工在线",
)
...
def classify_by_rules(message: str) -> Optional[IntentResult]:
    ...
    # 知识库查询优先于业务工具，但仅在无订单号时匹配
    if not order_no and _contains_any(normalized, KNOWLEDGE_KEYWORDS):
        return IntentResult(
            intent="knowledge_query", confidence=0.85,
